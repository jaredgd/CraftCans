...
1. Please ensure that you have the files in the DiscGolf_files folder within the `week10` folder.  
...
3. Create a new project ("new package with devtools") called `DiscGolf`.  Be sure to confirm that you are seeing both a "Build" tab and a "Git" tab.

4. Edit the `DESCRIPTION` file with your information (see the DESCRIPTION file I've provided for an example)
...
5. Save your changes to the `DESCRIPTION` file then run:

  `options(usethis.full_name = "YOUR NAME")`

  `usethis::use_mit_license()`

6. Run `usethis::use_data_raw()` to create the `data-raw` folder

7. Open the data-raw folder and DATASET.R then replace with contents of wrangling commands from the DATASET.R file I've provided.  Save this modified file.
...
8. Use the "Session" menu to change "working directory" to "source file location", then run 
"source" on the `DATASET.R` file.

9. Check that the `data` folder now contains the data `DiskGolf.rda`.

10. Copy the `DiscGolf.R` file (documentation that I've provided) to the R folder
...
11. Commit all of these files (see the "Git" tab)

12. Run `usethis::use_git()` to add other files
